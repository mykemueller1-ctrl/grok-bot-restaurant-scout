...
const rules = [
  ...
  { re: /(?:\$|€)\d+(?:\.\d{2})?(?:\s+(?!per\b)\w+){0,3}\s*(per[\s-]?(?:fulfilled\s+)?order|per[\s-]?delivery|\/\s*order)|per[\s-]?(?:fulfilled\s+)?order (fee|charge|service fee|overage|commission)|per[\s-]?delivery (fee|charge)|flat \$\d+(?:\.\d{2})?\s*(?:\+|fee)|online order service fee|(?:\$|€)\d+(?:\.\d{2})?\s+commission fee|flat rate \+ percentage|€0\.50/, signal: "per_order_platform_fee", w: 15 },
  // First-party OS still takes a % (Owner Flex 5% / IG checkout 5% / Flipdish ~2–7% direct commission)
  { re: /5%\s*(restaurant|per[\s-]?order|platform|order support|selling|checkout|charged on|fee|transaction)|7\.5%\s*(support local|of the subtotal|fee|guest|diner)?|2\s*[–\-]\s*7%\s*(per[\s-]?order\s*)?(commission|fee)|2\.99%\s*(service|online|ordering|platform|consumer)?|2\.5%\s*(order\s*)?processing|2\.5%\s*\+\s*\$0\.29|1\.9%\s*(service|fee)?|capped at \$2(?:\.00)?|max \$2(?:\.00)?|restaurant fee per order|order support fee|owner\.com.*5%|\$249.*5%|\$0\.40\s*(per[\s-]?order|minimum)|\$1\.99\s*minimum|hard cap of \$7\.50|cap of \$7\.50|flipdish.*(2|5|7)\s*%|online ordering service fee|support local fee|order with google fee|2%\s*\(charged to customer\)|1%\s*\(charged to customer\)|2%\s*convenience fee|1%\s*convenience fee|convenience fee per order|3%\s*on all orders|3%\s*platform commission|platform commission:\s*3%|5%\s*on all orders|5%\s*(fee\s+)?per\s+catering|catering\s+orders?\s+will\s+be\s+charged\s+a\s+5%|5%\s*(rate|commission|fee)\s*(through|on|per)?\s*(your\s+)?(own\s+)?(branded\s+)?(direct\s+)?catering/, signal: "first_party_pct_fee", w: 15 },
