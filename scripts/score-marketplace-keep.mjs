...
const rules = [
  ...
  { re: /reservation[\s-]?skew|no first[\s-]?party oo|opentable[\s\/-]*(reservation[- ]?)?skew|reservation[- ]only|make a reservation(?!.*\b(order|delivery|takeout)\b)|cult love without (shop|buy-now|first[\s-]?party)/, signal: "reservation_skew_no_buy_now", w: 20 },
];
