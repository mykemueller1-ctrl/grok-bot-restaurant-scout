...
2. **Public venue enrichment (adopted — Active, no Apollo required):**
   ...
   - **buy_now_path / order_rails** (same public pass): official site CTAs — first-party OO, Toast OO, Shopify merch, Resy/OpenTable, or marketplace leak (UberEats/DoorDash on brand site). Calibration: `fixtures/tool-hunt/*-love-brands.json` → `buy_now_path` (e.g. Michaels Genuine site → UberEats/DoorDash; Franklin → Shopify+preorder). Fold rails into `PainLead.opportunity_signals` + sales-lead `suggested_angle`
