...
3. For each of top 5, capture:
   ...
   - **order_rails / buy_now_path** (Active public second pass): official site + Composio `COMPOSIO_SEARCH_WEB` / `COMPOSIO_SEARCH_FETCH_URL_CONTENT` / `COMPOSIO_SEARCH_GOOGLE_MAPS` — note first-party OO, Toast OO, Shopify merch, Resy/OpenTable, or marketplace leak (UberEats/DoorDash on brand site → teach `brand_site_marketplace_leak`; OpenTable-only / no OO → teach `reservation_skew_no_buy_now`). Calibration: `fixtures/tool-hunt/*-love-brands.json` → `buy_now_path` on live primaries (Franklin Shopify+preorder; Michaels marketplace leak; Husk Toast gift+Resy; G&TG reservation-skew; Rioja Flavor Dojo).
