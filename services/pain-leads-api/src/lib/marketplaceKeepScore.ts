...
const RULES: { re: RegExp; signal: string; w: number }[] = [
  ...
  { re: /\$\d+(?:\.\d{2})?(?:\s+(?!per\b)\w+){0,3}\s*(per[\s-]?(?:fulfilled\s+)?order|\/\s*order)|per[\s-]?(?:fulfilled\s+)?order (fee|charge|service fee|overage|commission)|online order service fee|\$\d+(?:\.\d{2})?\s+commission fee/i, signal: "per_order_platform_fee", w: 15 },
  // First-party OS still takes a % (Owner Flex 5% / IG checkout 5% / Flipdish ~2–7% direct commission)
  { re: /5%\s*(restaurant|per[\s-]?order|platform|order support|selling|checkout)|7\.5%\s*(support local|of the subtotal|fee|guest|diner)?|2\s*[–\-]\s*7%\s*(per[\s-]?order\s*)?(commission|fee)|2\.99%\s*(service|online|ordering|platform|consumer)?|restaurant fee per order|order support fee|owner\.com.*5%|\$249.*5%|\$0\.40\s*(per[\s-]?order|minimum)|flipdish.*(2|5|7)\s*%|online ordering service fee|support local fee/i, signal: "first_party_pct_fee", w: 15 },
