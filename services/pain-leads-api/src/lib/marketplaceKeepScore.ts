...
const RULES: { re: RegExp; signal: string; w: number }[] = [
  ...
  { re: /5%\s*(restaurant|per[\s-]?order|platform|order support|selling|checkout)|2\s*[–\-]\s*7%\s*(per[\s-]?order\s*)?(commission|fee)|2\.99%\s*(service|online|ordering|platform|consumer)?|restaurant fee per order|order support fee|owner\.com.*5%|\$249.*5%|\$0\.40\s*(per[\s-]?order|minimum)|flipdish.*(2|5|7)\s*%|online ordering service fee/i, signal: "first_party_pct_fee", w: 15 },
