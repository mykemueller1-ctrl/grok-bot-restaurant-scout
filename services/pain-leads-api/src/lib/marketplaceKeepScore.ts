...
const RULES: { re: RegExp; signal: string; w: number }[] = [
  ...
  { re: /(?:\$|€)\d+(?:\.\d{2})?(?:\s+(?!per\b)\w+){0,3}\s*(per[\s-]?(?:fulfilled\s+)?order|per[\s-]?delivery|\/\s*order)|per[\s-]?(?:fulfilled\s+)?order (fee|charge|service fee|overage|commission)|per[\s-]?delivery (fee|charge)|flat \$\d+(?:\.\d{2})?\s*(?:\+|fee)|online order service fee|(?:\$|€)\d+(?:\.\d{2})?\s+commission fee|flat rate \+ percentage|€0\.50/i, signal: "per_order_platform_fee", w: 15 },
